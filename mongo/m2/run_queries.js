...
// run all queries limiting each to 3 results

mongoConn
    .db("m2")
    .collection("employees")
    .find({ "managers.emp_no": "110511" })
    .limit(3)
    .toArray((err, managerIdResults) => {
        if (err) throw err;
...
mongoConn
    ...
    .limit(3)
    .toArray((err, managerFirstNameResults) => {
        if (err) throw err;
...
mongoConn
    .db("m2")
    .collection("employees")
    .find({ "titles.title": "Senior Engineer" })
    .limit(3)
    .toArray((err, titleResults) => {
        if (err) throw err;
...
mongoConn
    .db("m2")
    .collection("employees")
    .find({ "depts.dept_name": "Development" })
    .limit(3)
    .toArray((err, deptNameResults) => {
        if (err) throw err;
