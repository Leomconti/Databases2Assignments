...
async function getEmployeesByManagerName(firstName, lastName) {
    const employeesResult = mongoConn
        ...
        .collection("employees")
        .find({
            $and: [{ "managers.first_name": firstName }, { "managers.last_name": lastName }],
        })
        .toArray();
    return employeesResult;
}
...
async function runQueries() {
    try {
        ...
        console.log("Employees by manager ID:", JSON.stringify(employeesByManagerId, null, 2));

        const employeesByManagerName = await getEmployeesByManagerName("Selwyn", "Cushing");
        console.log("Employees by manager name:", JSON.stringify(employeesByManagerName, null, 2));
    } catch (error) {
